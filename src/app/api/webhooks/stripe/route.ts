...
export async function POST(request: NextRequest) {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        try {
            ...
            // Country Mapping (ISO code -> CreativeHub Country ID)
            ...
            const countryId = COUNTRY_ID_MAP[countryCode] || 177;

            console.log(`Resolving Country: ${countryCode} -> ID: ${countryId}`);

            // Build CreativeHub order (PascalCase as per API docs)
            const creativeHubOrder: CreativeHubOrder = {
                ...
                OrderItems: cartItems.map((item) => {
                    ...
                    // Add CreativeHub IDs if available
